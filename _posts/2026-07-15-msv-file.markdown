...
date:   2026-07-15 11:19:00 +0800
categories: radio
---

# TODO ADD THE IMAGES

imma post this first, i really need source control for this plus i have started refrenceing to this and it's annoying having to open another vsc to look at it sooooooo gonna publish first

This is all over the place as im still in the process of figuring out exactly how each files are created.
...
Since the original entry was long enough, i thought may as well seperate them and go much more indepth and document things along the way. As you can see it quite long already so yeah use the toc as a guide to hop around this entry. I believe i got most of the files now? i cannot confirm anything will be exactly the same as yours, use this as a reference only. i'm a human and human can make mistakes. There are still a few files i did not figuired out but this only contains files that are required to manually writing the database just enough to list the tracks.

also there is a toc2somethingsomething file, I found out the hard way **that** file has a affect on cd playback. I did not looked into how it structure, but what i discovred is if the entry does not match up with the amount of "albums" in the MSV it **will** not list cd tracks _for some reason_. I **might** add it at a later date if i ever figured out. For now a temporary fix is to copy the same entry (about 412 bytes) and paste it a few times, change the ids and the total track (right next to file version).

## WARNING

This is **not** a complete documentation on the file structure, it is made based entirely on **assumptions and my own experiments**. Only use this as reference and experiment on your own, **keep backups** of the files. I'm **not** responsible for any data loss, create/modify at your own risk.
...
> Gemini said it is volume ? i don't trust it plus i tried, did absoutly nothing
...
> wait assuming the date is defaulted to the release date, this thing is just older than me by 3 months ???? _anyways_
...
> pioneer navi studio for other models suggest that it is used for album art that you can burn into a disc and it will add to it. Mine does not have that feature even though **it's a fucking black colored of hrz-88**
...
nvm it just create a new file for that artist
...
this is what i have figured out so far, might be inaccurate since im guessing based on the disc cue file i have. I away from home right now and don't have access to a disc drive.
...
struct tocentry{
    ...
    u8 totalMinutes;
    u8 totalSeconds;
    u8 totalMiliseconds; //(idk not sure)
    u8 paddingss;
    u8 unsure; //might be a fixed byte ?
    u16 paddingsss;
    
    trackOffset data[99];
    //u8 data[396]; //uncomment this and comment above if you don't wanna get confused
};
...
struct toc2opl{
    ...
    u16 totalEntry;
    u16 paddings;
    tocentry tocentry[while(!std::mem::eof())] [[comment("Tracks")]];
};
...
> Questions ? send me a mesage on discord or twitter or open a github issue, ill get back to you (maybe)

<p style="opacity:0; height:0;">I won't bite... most of the time</p>
...
As you can see it's been a while since my last entry even though this is mostly complete, i got so damn burned out from the projects, presentation and finals im not in the mood to even open vsc. I spend my entire break playing games with my friends, baking brownies and fixing some stuff (i know odd combo). Originally planned to release this along with the tool i have written but it's quite a mess so uh someday maybe. i can release it as is but even i got confused lol. As for the kiwi file, yeah i give up it's wayyyy too outside of my skill. If someone is interested in picking it up, i can send you the entire spec document (in english) as well as 2 sample kiwi map file and a work in progress hex pats.

One of my groupmate suddenly dropout in the middle of the semester so i ended up having to lead 3 groups + handle a project that was meant for 5 people but with just 2. Not to mention there is presentation for those project as well. _i ended up using claude for one of the project, i was no longer in the mood to program another website in under a week_

> thanks a lot, H██████

Since last semester I've been picking up games to play and ig it's fair to say I recovered now (ish) gonna start writing again sometimes soon™

> if you like puzzle games as much as i do (like portal series), please check out schrodinger's cat burglar on steam.

> I've been obsess with this game ever since I got it

<p style="opacity:0; height:0;">On a unrelated note, I got my psvr working perfectly fine and painless with my pc. Im so fucking happy if you can't tell</p>

Hope this help someone, maybe, probably not.
